...
def polyfit_using_prev_fit(warped_binary, left_fit, right_fit):
    # Declare margin
    margin = 60
...
def measure_curvature_real(left_fit, right_fit, warped_binary):
    ...
    left_curverad = ((1 + (2*left_fit[0]*y_eval*ym_per_pix + left_fit[1])**2)**1.5) / np.absolute(2*left_fit[0]) * curve_rad_scale_factor
    right_curverad = ((1 + (2*right_fit[0]*y_eval*ym_per_pix + right_fit[1])**2)**1.5) / np.absolute(2*right_fit[0]) * curve_rad_scale_factor

    return left_curverad, right_curverad

def measure_center_dist_real(warped_binary, left_fit, right_fit):
...
def process_image(img):
    ...
    # Fit the lane line into second order polynomials with the look ahead filter
    ...
    left_curverad, right_curverad = measure_curvature_real(left_fit, right_fit, warped_binary)
    center_dist = measure_center_dist_real(warped_binary, left_fit, right_fit)
